...
| ~~WP-156~~ | ~~`hooks/stop.py` registration uses absolute path~~ | ~~`.claude/settings.json`~~ | ✅ Resolved 2026-05-06. Replaced absolute `python3 /home/oliver/projects/graph-memory-fabric/hooks/stop.py` with `python3 "$CLAUDE_PROJECT_DIR/hooks/stop.py"` in `.claude/settings.json:152` — Claude Code injects `CLAUDE_PROJECT_DIR` into the hook process env at fire time (canonical pattern per `claude-plugins-official/plugin-dev/hook-development` references), so the registration now travels with the repo regardless of clone path. Docstring example in `hooks/stop.py:27–32` updated to match. Verified with a faithful `/bin/sh -c` simulation: hook fires correctly under both the canonical project path and an alternate clone path, and fails loudly (python3 stderr) when the env var is missing — which is the desired diagnostic outcome. |
