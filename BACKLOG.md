...
|-------|---------|-----|-------|-------|--------|----------------|------------|-------|
| 1 | R2 | WP-169 | OAuth protected-resource metadata for MCP discovery | H | L | 5.0 | ✅ WP-105 | **Implementation complete. Pending: AC-1 prod-deploy acceptance — `claude mcp list` must show `memory: ✓ Connected` from a fresh Claude Code session after deploying commit `91f3f19` to the homeserver.** 23/23 tests pass (15 unit + 8 integration) against live local stack. All code-side DoD gates green. Once AC-1 is confirmed, move to Completed. See detail below. |
...
**Status (2026-05-06):** Implementation complete. 23/23 tests pass (15 unit + 8 integration) against live local stack. Commit `91f3f19`. Awaiting AC-1 prod-deploy verification (`claude mcp list` on homeserver) before moving to Completed.

**Retrospective:**
- What went well: R-1 (Starlette route-registration precedence) resolved cleanly via empirical verification during implementation — the plan anticipated this risk, documented both fallback paths, and the primary path worked. No plan deviations of substance. The tight suffix allow-list approach is simple to audit and the test matrix covers it thoroughly. Splitting test concerns across four files (middleware, open-paths, metadata shape, integration) produces clean and independently runnable test groups.
- What to improve: The mock-target gotcha for deferred `settings` imports is worth capturing as a project-level pattern note. `mcp_auth.py` imports `settings` inside `__call__` via `from memory_service.config import settings` (deferred import to avoid circular import at module load), but this means unit tests must patch `memory_service.config.settings` rather than `memory_service.mcp_auth.settings` — the mock target is the config module, not the calling module. Same pattern applies in `auth.py`. Future test authors should be aware of this: patch at the module where the object is defined, not where it is used. See the `_run_middleware` helper in `test_wp169_mcp_auth_allowlist.py` and `verify_api_key` test in `test_wp169_open_paths.py` for the correct pattern.
