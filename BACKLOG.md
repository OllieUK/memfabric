...
|----------|---------|----|-------|-------|--------|----------------|------------|-------|
| 1 | R1 | WP-022 | Cap neighbour count in search results | M | L | 2.0 | WP-005 ✅ | `collect(DISTINCT n.id)` unbounded with `max_hops=3` on dense graph — add slice cap (e.g. `[..50]`). Correctness risk as graph grows. |
...
| 6 | R1 | WP-038 | Memory lifecycle operations — update, merge, archive | H | H | 1.0 | WP-037 ✅ | First-class memory maintenance: PATCH, merge, archive, restore. Kept immediately ahead of WP-039 because it is the prerequisite for the stronger next branch. See detail below. |
...
| 22 | R3 | WP-042 | Self-contained `memory_client` packaging | L | L | 1.0 | WP-031 ✅ | Move `pyproject.toml` into `memory_client/` for independent install. Re-scored from medium value because it is packaging polish rather than core product capability. |
| 23 | R3 | WP-008 | LLMClient abstraction | M | M | 1.0 | WP-007 ✅ | `LLMClient.ask(system, prompt, model)` wrappers for Claude/OpenAI/Ollama. |
| 24 | R3 | WP-009 | Headless agent framework | M | H | 0.67 | WP-008 | `BaseAgent` using `memory_client` + `LLMClient`; scheduled/event-driven tasks. |
| 25 | R3 | WP-010 | Remote/mobile access | L | H | 0.33 | WP-009 | Tailscale/VPS hosting + TLS + API key auth. |
| 26 | R3 | WP-011 | Custom graph-cloud UI | L | H | 0.33 | WP-006 | React + D3.js/vis-network consuming `GET /memory/graph`. |
