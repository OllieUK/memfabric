...
| ID | Title | Started |
|----|-------|---------|
| WP-138b | Apply calibrated dedup threshold to existing Threat corpus | 2026-05-06 | **Implementation complete 2026-05-07.** `merge_threat()` + `POST /knowledge/threats/{id}/merge` + `scripts/apply_threat_dedup_wp138b.py` written, 21/21 tests passing. **Pending: production dry-run + merge pass on homeserver** (run `python3 scripts/apply_threat_dedup_wp138b.py --dry-run` then without `--dry-run` on homeserver where Bolt port is reachable). |
...
| WP-167 | Bake `scripts/` directory into the api Docker image | `Dockerfile` | Surfaced 2026-05-06 in `backup-nightly.sh` first run. The Dockerfile copies `memory_service/`, `memory_client/`, `mcp_server/`, and `pyproject.toml` into `/app` but **not** `scripts/`. Operational scripts that need the venv environment (`dump_db.py`, `restore_db.py`, `seed_strands.py`, `init_schema.py`, `init_knowledge_schema.py`, the various ingest scripts) are inaccessible via `dcumf exec` because `/app/scripts/` doesn't exist. Workaround in place: backup-nightly.sh `docker cp`s `dump_db.py` into the container per run. Proper fix: add `COPY scripts/ ./scripts/` to the Dockerfile after the existing COPYs. Trade-off: minor image-size bump (~50 KB of Python). Win: makes every script in `scripts/` available via `dcumf exec memfabric-api python3 /app/scripts/<name>.py` without per-script docker-cp dance. After this lands, simplify `backup-nightly.sh` to drop the docker-cp workaround. |
...
| WP-172 | Add `?include_archived=false` filter to `GET /knowledge/threats` | `memory_service/knowledge_repo.py`, `memory_service/knowledge_routes.py` | Surfaced 2026-05-07 in WP-138b /simplify review (R4). `knowledge_repo.list_threats` returns all Threat nodes with no `archived` filter. After the WP-138b merge pass, `GET /knowledge/threats` returns archived nodes alongside live ones. Add `include_archived: bool = False` query param — default excludes archived nodes, which is the expected caller behaviour. Pick up when next touching `knowledge_routes.py`. |
