...
| 16 | R2 | WP-SEC-3 | Ingestion provenance + STIX hash-pin TODO + ingest guard module | M | M | 1.0 | WP-SEC-2 ✅ | Populate `data/frameworks/SOURCES.md` and `data/threats/SOURCES.md` with SHA-256 + provenance for every existing framework file. Add STIX hash-pin TODO comment to `scripts/ingest_attack.py`. Create `memory_service/ingest_guard.py` reusing `hooks/_filters.py`. Wire the guard into `scripts/ingest_document.py`, `scripts/ingest_framework.py`, `scripts/extract_cti_threats.py` at chunk-write time; quarantine flagged chunks to `data/ingest-quarantine/<sha256>.txt`. See plan file for full spec. |
