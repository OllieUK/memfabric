...
| 7 | R2 | WP-180 | Remove `scripts/` deprecation shim layer (WP-173 follow-up) | L | L | 1.0 | WP-173 ✅ | One cycle after WP-173, delete the 42 deprecation-shim files in `scripts/` that proxy to `cyber_knowledge/ingest/*` modules. All callers should have migrated to `python -m cyber_knowledge.ingest.X` by then. Update any operational wrappers still hitting the old paths (none expected). |
