...
| 7 | R2 | WP-096 | API authentication (bearer tokens / API keys) | H | M | 1.7 | — | FastAPI middleware to authenticate all requests via bearer token or `X-API-Key` header. Enables safe external exposure of the service. See detail below. |
...
| 17 | R2 | WP-024 | `cleanup_nodes` support multiple ids per label | L | L | 1.0 | — | Change `extra_ids: dict[str, str]` to `dict[str, str \| list[str]]` for multi-node cleanup in tests. Required by WP-076. |
...
**Confirmed high-urgency (2026-04-08).** Integration tests write real memories to the live graph and are not cleaning up after themselves. In a single dedup review session, 16 test artefact memories were found across 4 test runs (2026-04-04 to 2026-04-07), all polluting the live companion graph. They flooded the near-duplicate queue with noise and required a manual `DETACH DELETE` directly against Memgraph — bypassing the API entirely. This will happen again after every integration test run until this WP is implemented.

Without explicit ephemeral semantics, test artefacts accumulate and corrupt companion context. Need: test memories excluded from normal retrieval + auto-cleaned once no longer needed.
