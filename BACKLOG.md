...
| 7d | R2 | WP-184 | `create_constraint` output: `[SKIP]` on Memgraph idempotent re-create | L | L | 1.0 | WP-174 ✅ | Memgraph's `CREATE CONSTRAINT` is silently idempotent — re-running on an existing constraint succeeds without raising `already exists`. The `create_constraint` helper in `cyber_knowledge/ingest/schema_utils.py` therefore prints `[OK] Constraint created` on re-runs when it should print `[SKIP] Constraint already exists`. Functionally correct (idempotent in effect) but cosmetically misleading. Surfaced during WP-174 live-stack verification 2026-05-12. Fix: `SHOW CONSTRAINT INFO` before/after, or check the run summary's `constraints-created` counter where Memgraph exposes one. |
