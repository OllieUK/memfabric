...
|----------|---------|----|-------|-------|--------|----------------|------------|-------|
| 1 | R1 | WP-079 | Importance recalibration pass | H | L | 3.0 | — | One-time LLM-assisted review of the ~173 memories currently rated 4 or 5, applying the blast-radius-of-absence definition adopted 2026-03-28. Downgrade any memory that does not clear the bar for its current level. Goal: restore wake-up signal quality and correct the mechanical side-effect of inflation (inflated strength floors, slow decay on non-critical memories). No code change — data-only. |
...
| 27 | R2 | WP-077 | Extract shared schema-init utilities | L | L | 1.0 | WP-069 | `create_constraint()` and `get_embedding_dimension()` are copy-pasted identically in `init_schema.py` and `init_cybersec_schema.py`. Extract to `scripts/schema_utils.py`. Found in WP-069 /simplify review. |
| 28 | R2 | WP-025 | Extract shared CLI error handler | L | L | 1.0 | — | 4+ identical `except httpx.*` blocks in `cli.py`. Extract once. |
| 29 | R2 | WP-026 | `MemoryType` mirror in `memory_client` | L | L | 1.0 | WP-007 ✅ | Mirror enum so callers get IDE completion without cross-package import. |
| 30 | R2 | WP-023 | Extract `get_session` context manager for 503 handling | L | L | 1.0 | WP-029 ✅ | `try/with driver.session()/except ServiceUnavailable→503` copy-pasted across all endpoints. Do after WP-029 (adds more endpoints). |
| 31 | R2 | WP-020 | UNWIND for person/strand/related_ids writes | L | L | 1.0 | WP-004 ✅ | Replace per-item `session.run()` loops in `add_memory` with UNWIND queries. Add `related_ids` max-length cap (e.g. 20). |
| 32 | R2 | WP-021 | Non-blocking embedding in async endpoints | L | L | 1.0 | WP-004 ✅, WP-005 ✅ | `get_embedding()` blocks the event loop. Wrap with `run_in_executor` when concurrent usage becomes a problem. |
| 33 | R2 | WP-024 | `cleanup_nodes` support multiple ids per label | L | L | 1.0 | — | Change `extra_ids: dict[str, str]` to `dict[str, str \| list[str]]` for multi-node cleanup in tests. Required by WP-076. |
| 34 | R2 | WP-017 | Embedding cache eviction / size cap | L | L | 1.0 | WP-003 ✅ | `EMBEDDING_CACHE_DIR` grows without bound. Add LRU eviction or max-entry cap. |
