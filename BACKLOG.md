...
| 2 | R2 | WP-105 | Streamable HTTP MCP transport (ADR-003) | H | M | 1.5 | WP-096 ✅ | Replace stdio `memory-mcp` with an MCP endpoint mounted at `/mcp` inside the FastAPI service. Mount FastMCP as ASGI sub-app; refactor 27 tools to call repo functions directly (remove MemoryClient/httpx); wire `verify_api_key` as FastMCP dependency; consolidate `ENABLE_KNOWLEDGE_LAYER` to single config source; update Claude Desktop and `.mcp.json` configs to `url`-based entries; keep stdio entrypoint as offline fallback. Eliminates local-install requirement on all harnesses; removes internal HTTP round-trip on every tool call; reduces auth surface to one. See ADR-003. |
