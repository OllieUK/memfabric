...
|----|-------|---------|-------|
| WP-174 | ADR-006: Asset and Policy node model + OSCAL parameter handling | 2026-05-12 | Schema-only WP. Drafts ADR-006 defining `AssetClass`, `Policy`, `PolicySection` node labels and the OSCAL `params` binding model. Class-based asset taxonomy (industry sub-axes deferred). Unblocks WP-175/176/179. |
...
> Within a large equal-score block, order full dependency chains before standalone items. Rank chains by total chain value (`H=5`, `M=3`, `L=1` summed across the chain), then standalone `M/M` items, then standalone `L/L` items. Preserve existing order within each subgroup unless a dependency requires a move.
...
> **2026-05-11 — order numbering note:** Following the WP-173 / WP-113 close-out on 2026-05-11, the cyber-layer slate (WP-174 → WP-180) was inserted as the new top of the queue per ADR-005 follow-on plan. The remaining rows below have NOT been re-numbered in this revision — read the Priority score and Depends on columns for execution order; the Order column from row 8 onwards reflects pre-insertion numbering and will be re-aligned in the next BACKLOG-maintenance pass. Top-of-queue (Orders 1–7) is canonical.

| Order | Release | ID | Title | Value | Effort | Priority score | Depends on | Notes |
|-------|---------|-----|-------|-------|--------|----------------|------------|-------|
| 1 | R2 | WP-174 | ADR-006: Asset and Policy node model + OSCAL parameter handling | M | L | 1.7 | ✅ WP-173 | Defines new node labels (`AssetClass`, `Policy`, `PolicySection`) and the OSCAL parameter model used by GS++ ingest. Class-based asset taxonomy (NOT a CMDB replacement) per Oliver 2026-05-11; business-type/industry sub-axes deferred. Policy/PolicySection as first-class labels with edges to Precept and Framework leaves. Defines how OSCAL `params` (`{{ insert: param }}`) bind to org-specific values via policies. Unblocks WP-175, WP-176, WP-179. Schema-only WP — no seeding, no ingestion. |
| 2 | R2 | WP-175 | Generic OSCAL Catalog ingest pipeline | M | M | 1.0 | WP-174 | OSCAL 1.1.3 Catalog reader at `cyber_knowledge/ingest/oscal/` mapping OSCAL group→Framework branch, control→Framework leaf, control.parts.statement+guidance→Chunk, control.props.sec_level + effort_level→Framework leaf properties, control.params→Param model per ADR-006. Vendor or thin-wrap `oscal-pydantic`. Reusable for any OSCAL-published standard (GS++, future SP 800-53 re-ingest, etc.). |
| 3 | R2 | WP-176 | BSI Grundschutz++ (GS++) OSCAL ingest | M | M | 1.0 | WP-175 | Apply WP-175 to the CC-BY-SA-4.0 BSI GS++ catalog at github.com/BSI-Bund/Stand-der-Technik-Bibliothek. 20 top-level domains, 166 groups, 998 controls. German-language; multilingual embedding model handles natively. Supersedes WP-133 (PDF-scrape Kompendium ingest) — GS++ replaces the Kompendium per BSI roadmap (pilot 2026-04, public 2026-10, certification 2027-01). Sec_level + effort_level give built-in 2-axis prioritisation grid for maturity scoring. |
| 4 | R2 | WP-177 | CISA KEV (Known Exploited Vulnerabilities) ingestion | M | M | 1.0 | WP-174 | Daily-refresh ingestion of the CISA KEV catalog. New `CVE` node label, KEV flag with timestamp, ATT&CK technique mapping where present. Pairs with WP-178 (D3FEND) to provide a live "what's being exploited / what mitigates it" axis complementing the strategic SABSA path. Distinct from WP-108 ThreatReport ingestion: KEV is structured, machine-readable, authoritative on actual in-the-wild exploitation. |
| 5 | R2 | WP-178 | D3FEND defensive technique ingestion | M | M | 1.0 | WP-108 ✅ | (Promoted from former Order 12 / old WP-114.) Ingest MITRE D3FEND (~500 defensive techniques) from OWL/JSON-LD ontology. Create Framework nodes (level=defensive-technique) under a D3FEND root. Create `d3f:counters` edges as MITIGATES edges D3FEND → ATT&CK. Use D3FEND's own SP 800-53 / NIST CSF references for INFORMS edges. Adds defensive pathway alongside M-Series for SOC-level traversal; closes the loop with KEV for the pressure engine. |
| 6 | R2 | WP-179 | Pressure-engine endpoint: `GET /pressure/business-attribute` | M | M | 1.0 | WP-177, WP-178 | First consumer-facing scoring endpoint. Combines KEV freshness × ATT&CK linkage × INFORMS-density-weighted Framework leaves × BusinessAttribute via INFLUENCE. Returns ranked list of pressured BAs. Proof-of-life for use case 1 (threat-and-compliance-aware maturity review). Lays the substrate for use cases 2 and 3 (policy review against landscape change; policy architecture build). |
| 7 | R2 | WP-180 | Remove `scripts/` deprecation shim layer (WP-173 follow-up) | L | L | 1.0 | WP-173 ✅ | One cycle after WP-173, delete the 42 deprecation-shim files in `scripts/` that proxy to `cyber_knowledge/ingest/*` modules. All callers should have migrated to `python -m cyber_knowledge.ingest.X` by then. Update any operational wrappers still hitting the old paths (none expected). Also covers the `scripts/schema_utils.py` shim added in WP-174 — same removal path. |
| 7a | R2 | WP-181 | Revert dead `memfabric-db` port-bind in `docker-compose.yml` (WP-174 follow-up) | L | L | 1.0 | WP-174 | The `ports: ["127.0.0.1:7687:7687"]` binding added in commit `b0b0d40` provides no useful access because `/etc/ssh/sshd_config` has `AllowTcpForwarding no`. Remove the two lines so prod doesn't carry a useless port exposure. Single-line revert + GitOps deploy. |
| 7b | R2 | WP-182 | Refactor `schema_init.py` vector-index creation into a table-driven loop | L | L | 1.0 | WP-174 | Seven near-identical 18-line blocks (ctrl/framework/chunk/threat/business_attribute/policy/policy_section embedding indexes) flagged by /simplify on WP-174. Deferred from WP-174 because unifying touches all seven blocks and would make the additive WP-174 diff much harder to review. Mechanical: loop over `[(name, label, capacity_attr), ...]` calling `create_vector_index` then `validate_vector_index`. Collapses ~130 lines to ~25. |
| 7c | R2 | WP-183 | Promote `_get_constraints` / `_get_vector_indexes` to shared test helper | L | L | 1.0 | WP-174 | `tests/cyber_knowledge/test_schema_init_integration.py` and `tests/test_wp069_knowledge_schema.py` carry near-identical implementations of these two Memgraph-introspection helpers. Move to `tests/_schema_introspection.py` (or `tests/conftest.py` fixtures) and have both call sites import. Acts as anchor for future schema-affecting WPs (WP-175+). |
| 8 | R2 | WP-134 | BSI Lagebericht 2025 ingestion as threat intelligence document | M | M | 1.0 | ✅ WP-108, ✅ WP-113 | Ingest the BSI Lagebericht 2025 ("Die Lage der IT-Sicherheit in Deutschland 2025") as a ThreatReport document into the knowledge layer. The report is web-first (no single download PDF); primary URL: `https://medien.bsi.bund.de/lagebericht/`. Approach: (1) fetch the report's HTML pages via `ingest_document.py` or a custom web-scraping script, chunking by section; (2) create a `ThreatReport` document node and `Threat` nodes for named threat actor groups and attack categories mentioned; (3) wire `JEOPARDISES` edges from Threats to relevant Precept nodes (WP-113) and `MAPPED_TO_TECHNIQUE` edges to ATT&CK nodes where technique IDs are cited. The 8-page PDF handout (`/lagebericht/Lagebericht2025_Achtseiter.pdf`) can serve as a starting point for key statistics and threat categories before tackling full web ingestion. German language — multilingual embedding model handles this natively. |
| 9 | R2 | WP-145 | CalDAV ↔ Fabric bi-directional task sync | M | M | 1.0 | WP-143 ✅ | `tools/sync_caldav.py` — syncs first-order Task nodes (project tasks + external tasks) between Nextcloud CalDAV and the Fabric. WPs (`source_ref=*:WP-*`) stay in Fabric only. Inbound: completed VTODOs → fabric `done`; new VTODOs → new Task nodes; `X-FABRIC-TASK-ID` property links VTODO to fabric UUID. Outbound: first-order Task nodes without a matching VTODO get pushed. CHANGELOG write-back: on inbound completion, appends entry to project-specific `CHANGELOG.md`. See detail below. |
| 4 | R2 | WP-146 | Windows Task Scheduler entry for CalDAV sync | L | L | 1.0 | WP-145 ✅ | WSL cron only fires while the WSL instance is active. A Windows Task Scheduler entry calling `wsl -e bash -c "python3 /home/oliver/projects/graph-memory-fabric/tools/sync_caldav.py"` ensures sync survives Windows restarts without requiring an open terminal. Trigger: on logon + every 4 hours. |
| 5 | R2 | WP-085 | **Analytics Phase — Sprint 1:** graph-vs-vector diagnostics, cluster discovery, bridge detection (WP-057 + WP-058 + WP-059) | M | M | 1.0 | WP-029 ✅ | Three tightly related graph-analytics capabilities best built together as a shared diagnostic layer: (1) graph-vs-vector agreement — compare each memory's nearest embedding neighbours with its actual `RELATED_TO`/`LEADS_TO` neighbourhood to surface where the graph lags or overlinks semantic reality; (2) latent cluster discovery — cluster embeddings offline to discover emergent themes and compare them with explicit `Strand` assignments to identify overly broad, missing, or mislabeled strands; (3) bridge-memory detection — identify memories that span otherwise separate embedding clusters or graph communities, surfacing high-leverage cross-domain connectors. All three share the same embedding-space traversal infrastructure and diagnostic output pattern. |
| 6 | R2 | WP-006 | Wire `GET /memory/graph` | M | M | 1.0 | WP-028 ✅, WP-029 ✅ | Filtered subgraph export: project/agent/tag/since/until params; returns `{nodes, edges}`. |
| 7 | R2 | WP-116 | Migrate episodic Memory embeddings to `paraphrase-multilingual-MiniLM-L12-v2` | M | M | 1.0 | WP-094 ✅ | WP-094 gave the knowledge layer its own `KNOWLEDGE_EMBEDDING_MODEL` (`paraphrase-multilingual-MiniLM-L12-v2`) but deliberately left episodic Memory nodes on `all-MiniLM-L6-v2`. Migrating Memory nodes to the same multilingual model enables cross-lingual episodic search, aligns the two embedding spaces (closer cos-sim between memory and knowledge layer nodes), and removes a class of subtle retrieval errors when searching in non-English. Migration requires: (1) Drop and recreate `mem_embedding_idx` at the new model's dimension (768 vs 384 for MiniLM-L6-v2; confirm exact dim for paraphrase-multilingual). (2) Re-embed all Memory nodes in batches via `scripts/migrate_embeddings.py` (already exists for the knowledge layer migration). (3) Update `EMBEDDING_MODEL` default in `.env.example`. (4) Run long-rest after migration to rebuild RELATED_TO edges against the new vector space. See detail below. |
| 8 | R2 | WP-128 | Tiered search MCP tool (compact index + selective detail fetch) | M | M | 1.0 | ✅ WP-127 | Add GET /memory/search/index (compact result set: id, fact excerpt, importance, strength) and POST /memory/batch (full MemoryResponse for selected IDs). Update MCP tool descriptions to document the two-step scan-then-fetch pattern. Reduces token cost on every search that doesn't need full detail. See detail below. |
| 9 | R2 | WP-125 | Weighted hop-traversal for wake-up anchor sections | M | M | 1.0 | WP-049 ✅ | Extend `wake_up()` companion_anchors and conversant_anchors beyond direct `ABOUT` edges. After retrieving direct ABOUT-linked memories, expand one additional hop via `RELATED_TO` edges, scoring candidates as `importance × strength × edge_weight^hop`. Direct ABOUT hits (hop=0) score at full value; 1-hop neighbours are weighted by the edge's Hebbian activation weight, so only strongly co-activated memories survive. Config: `WAKE_UP_COMPANION_ANCHOR_HOPS` (default 1), `WAKE_UP_CONVERSANT_ANCHOR_HOPS` (default 2). Rationale: conversant section benefits from deeper context about the person (e.g. project memory RELATED_TO a habit memory ABOUT Oliver) while companion (agent identity) stays shallower. Cross-section dedup already in place; hop-scored ordering integrates naturally into the existing priority chain. Marabot equivalent: `wakeup_max_hops` / `_query_person` — but implemented here via direct Cypher rather than a list-position proxy. |
| 10 | R2 | WP-139 | CLI and API shared-infrastructure DRY pass | M | M | 1.0 | — | Supersedes WP-023, WP-025, WP-026, WP-020, WP-021. Bundles five individually-scoped cleanup WPs targeting the same infrastructure spine (`main.py`, `knowledge_routes.py`, `cli.py`, `memory_repo.add_memory`) into a single focused DRY pass. See detail below. |
| 11 | R2 | WP-137 | German-language CTI extraction support for BSI Lagebericht | M | M | 1.0 | WP-108 ✅ | The BSI IT-Sicherheitslage report is German-language; `extract_cti_threats.py` currently yields 0 threats because all `BEHAVIOR_INDICATORS` and `TECHNIQUE_KEYWORDS` are English. The `paraphrase-multilingual-MiniLM-L12-v2` embedding model already handles German natively, so embeddings and deduplication will work correctly once sentences are extracted — the gap is solely in the extraction gate. **Option analysis:** **(1) Extend keyword/verb lists (lowest effort):** Add German behaviour-verb equivalents to `BEHAVIOR_INDICATORS` (e.g. "verwendet", "eingesetzt", "ausgeführt", "verschlüsselt") and German keyword→ATT&CK mappings to `TECHNIQUE_KEYWORDS` (many terms are loanwords: "ransomware"→T1486, "phishing"→T1566 already work; add "anmeldedaten"→T1003, "seitwärtsbewegung"→T1021, etc.). Single-pass pipeline preserved. Brittle: misses conjugations and paraphrases; requires ongoing manual curation for each new German report. **(2) Offline pre-translation via `argostranslate` (clean separation):** Translate extracted PDF text from `de→en` before the existing pipeline runs it. `argostranslate` runs fully offline (~100 MB model). `extract_cti_threats.py` stays English-only; translation is a composable pre-processing flag (`--translate-from de`). Downside: translation quality affects extraction accuracy; adds model download to setup. **(3) spaCy German NER + verb extraction (more robust extraction gate):** Use `spaCy` with `de_core_news_sm` to extract sentences containing action verbs, replacing the hand-curated `BEHAVIOR_INDICATORS` list. Handles conjugations automatically. TECHNIQUE_KEYWORDS mapping still needs German terms, but the behaviour-indicator gate becomes language-agnostic. Higher setup cost; adds spaCy as a dependency. **(4) Language detection + routing (polyglot architecture):** Add `langdetect` or `lingua` to detect language per-page, then route through a language-specific extractor module. Makes the pipeline polyglot by design — future French/Dutch/Japanese reports require only a new extractor, not a fork of the main script. Best long-term architecture if more non-English reports are anticipated. **(5) Offline LLM extraction via Ollama (highest effort, most robust):** Run a local model (e.g. `mistral`, `llama3`) as a pre-processor that produces structured `{sentence, techniques[]}` JSON directly from each page. Handles language, paraphrase, and technique mapping in one pass. Consistent with the "no external LLM API calls" constraint. Highest setup cost; slowest at ingest time. **Recommended path:** Option 2 (argostranslate) for a quick win on the BSI report; Option 4 (language detection + routing) as the target architecture if additional non-English reports are added. Surfaced during live ingestion run (0 threats from BSI report — 364 German-language sentences extracted but none passed the English extraction gate). |
| ~~12~~ | ~~R2~~ | ~~WP-114~~ | ~~D3FEND defensive technique ingestion~~ — **superseded by WP-178** (promoted to Order 5, identical scope) | — | — | — | — | Superseded 2026-05-11. See WP-178. |
| 13 | R2 | WP-113b | C2M2 (Cybersecurity Capability Maturity Model) ingestion | M | M | 1.0 | WP-113 | Split out from WP-113 Component 3 (2026-05-06). 10 domains with maturity-level descriptors. To assess: ingest as Framework nodes vs treating as Control-node metadata. Wire INFORMS to existing Framework leaves where embedding similarity is high. See detail below. |
| 14 | R2 | WP-113c | Zero Trust Architecture (NIST SP 800-207) ingestion | M | M | 1.0 | WP-113 | Split out from WP-113 Component 3 (2026-05-06). Principles rather than controls — likely cleanest as additional universal Precepts (added to WP-113's Component 1 set), with a small Framework spine for the document structure. See detail below. |
| 15 | R2 | WP-113d | CIS Controls v8 ingestion | M | M | 1.0 | WP-113 | Split out from WP-113 Component 3 (2026-05-06). 18 controls with implementation groups (IG1/IG2/IG3). Strong overlap with SP 800-53 / NIST CSF; may serve as a better "glue" between ATT&CK and governance frameworks than SP 800-53 for some use cases. See detail below. |
| 16 | R2 | WP-140 | WP-049 code-review follow-ups | L | L | 1.0 | ✅ WP-049 | Supersedes WP-122, WP-123, WP-124. Bundles three deferred code-review items from WP-049 (endpoint ordering assertions, empty ABOUT-edge unit tests, `.env.example` docs for WAKE_UP limits) into one WP. See detail below. |
| 17 | R2 | WP-141 | Ingest script hygiene | L | L | 1.0 | ✅ WP-108, ✅ WP-073 | Supersedes WP-135, WP-136, WP-097. Bundles three small improvements to `scripts/` surfaced from WP-108 and WP-073 simplify reviews (shared `ApiSettings` base class, optional `embedding` field on `ThreatCreate`, H1 heading support in `chunk_markdown`). See detail below. |
| 18 | R2 | WP-042 | Self-contained `memory_client` packaging | L | L | 1.0 | WP-031 ✅ | Move `pyproject.toml` into `memory_client/` for independent install. Re-scored from medium value because it is packaging polish rather than core product capability. |
| 19 | R2 | WP-130 | Concept tags on Memory nodes | L | L | 1.0 | — | Add `concepts[]` array property alongside `tags[]` to capture epistemic role of a memory (gotcha, pattern, trade_off, decision_rationale, how_it_works, why_it_exists, what_changed, problem_solution, open_question). Enables queries like "all gotchas about Memgraph" independently of subject tags. See detail below. |
...
| WP-115 | Refactor COBIT→ISO/NIST blocks to loop | `cyber_knowledge/ingest/cross_framework_informs.py` (was `scripts/create_cross_framework_informs.py` pre-WP-173) | When next touching that script |
...
| WP-SEC-R15b | Add SHA-256 pin verification to `ingest_attack_mitigations.py` | `cyber_knowledge/ingest/attack_mitigations.py` (was `scripts/ingest_attack_mitigations.py` pre-WP-173) | When next touching that script (same pattern as WP-SEC-R15 in `cyber_knowledge/ingest/attack.py`) |
...
| ~~WP-156~~ | ~~`hooks/stop.py` registration is not portable across hosts/OSes~~ | ~~`.claude/settings.json`~~ | ✅ Resolved 2026-05-06 in two steps. **Step 1 (commit `b20a8c8`):** replaced absolute `python3 /home/oliver/projects/graph-memory-fabric/hooks/stop.py` with `python3 "$CLAUDE_PROJECT_DIR/hooks/stop.py"` — adequate for Linux/WSL but exposed the deeper issue when fired from Windows-side Claude Code. **Step 2 (this commit):** replaced the shell-substituted form with an OS-aware inline Python launcher: `python3 -c "import os,runpy,sys; d=os.environ.get('CLAUDE_PROJECT_DIR'); sys.exit(0) if not d else runpy.run_path(os.path.join(d,'hooks','stop.py'), run_name='__main__')"`. Python builds the path natively via `os.path.join` and opens it via `runpy.run_path` — no shell-level path interpretation, so UNC paths (Windows: `\\wsl.localhost\Ubuntu-22.04\...`) and POSIX paths (Linux) both work without escaping or per-OS branching. Falls back to silent-skip when `$CLAUDE_PROJECT_DIR` is unset, matching the script's existing service-unreachable silent-skip philosophy. Verified end-to-end: Windows `python3` (UNC path), WSL `python3` (POSIX path), empty env var, unset env var — all four exit 0 with the expected behaviour. (Note: takes effect after Claude Code restart — the previous registration is cached in the running process.) |
...
| WP-166 | Memgraph backup hardening — off-host, restore-tested, dedicated user, alerting | `scripts/homeserver/backup-nightly.sh`, `docs/operations/backup.md`, new infra | Surfaced 2026-05-06 from the volume-orphaning near-miss. A minimum-viable nightly backup is in place (Layer 1: local JSON dump + volume tarball, 14-day retention, runs as root, cron-installed). The proper version covers the gaps documented in `docs/operations/backup.md`: (1) **off-host replication** — rsync or restic the daily artefacts to a separate machine on the LAN (laptop, NAS, second host). **Note 2026-05-06**: an existing rsync-based off-device backup pipeline was surfaced while installing the memfabric cron; investigate that first before designing new mechanism — extending the existing pipeline to cover `/opt/stacks/backups/memfabric/` may be the cleanest solution; (2) **restore-test discipline** — periodic automated restore-from-backup into a throwaway Memgraph instance to verify backups are actually usable; (3) **dedicated stack-user** — backup currently runs as root for parity with other host backups; should eventually be a dedicated user with read-only access to source dirs and write-only access to backup dirs, similar to `claude-diag` discipline; (4) **cold / off-site storage** — encrypted upload to Backblaze B2/S3 for ransomware and physical-loss defence; (5) **alerting on failure** — currently silent failures land in `backup.log`; want at least an email or push notification on any non-zero exit. Optionally also: tiered retention (7 daily + 4 weekly + 12 monthly) instead of flat 14-day; integration with the existing `dump_db.py` / `restore_db.py` pair to make sure they keep working as the schema evolves; a `make backup` target as a developer-facing shortcut. |
| WP-167 | Bake `scripts/` directory into the api Docker image | `Dockerfile` | Surfaced 2026-05-06 in `backup-nightly.sh` first run. The Dockerfile copies `memory_service/`, `memory_client/`, `mcp_server/`, and `pyproject.toml` into `/app` but **not** `scripts/`. Operational scripts that need the venv environment (`dump_db.py`, `restore_db.py`, `seed_strands.py`, `init_schema.py`, the various ingest scripts) are inaccessible via `dcumf exec` because `/app/scripts/` doesn't exist. Workaround in place: backup-nightly.sh `docker cp`s `dump_db.py` into the container per run. Proper fix: add `COPY scripts/ ./scripts/` to the Dockerfile after the existing COPYs. Trade-off: minor image-size bump (~50 KB of Python). Win: makes every script in `scripts/` available via `dcumf exec memfabric-api python3 /app/scripts/<name>.py` without per-script docker-cp dance. After this lands, simplify `backup-nightly.sh` to drop the docker-cp workaround. **Note (WP-173, 2026-05-11):** the cyber ingest scripts are now baked in via `COPY cyber_knowledge/ ./cyber_knowledge/` in the Dockerfile and are runnable as `python -m cyber_knowledge.ingest.<name>`. WP-167 remains relevant for the episodic-memory and infrastructure scripts that stayed in `scripts/` (dump_db, restore_db, seed_strands, init_schema, migrations, maintenance_runner). |
| WP-168 | Fix executable-bit discipline for shell scripts in repo | `.git/config` (local), `.gitattributes` or pre-commit hook | Surfaced 2026-05-06 from recurrent "chmod +x after every git pull" friction. `.git/config` has `core.filemode = false` (legacy from a checkout on a filesystem that didn't support exec bits — likely a Windows-mounted volume historically). With this flag, `chmod +x` at the filesystem level is invisible to `git add`, so shell scripts get committed as `100644` and need manual chmod at every install/deploy. Both `claude-diag` and `backup-nightly.sh` had the same install-time chmod ritual. Commit `b631b59` retroactively fixed those two files via `git update-index --chmod=+x`. Three resolution options for future scripts: (1) flip `core.filemode = true` in this repo's `.git/config` so future `chmod +x` is honoured (only affects this clone — anyone else cloning still hits the same trap unless their git config defaults differ); (2) add a `.gitattributes` rule, but `.gitattributes` doesn't actually cover exec bits — only EOL/merge/diff strategy — so this won't help; (3) add a pre-commit hook that runs `git update-index --chmod=+x` on any newly-staged `*.sh` file under `scripts/`. Option 3 is the durable answer; option 1 is a quick win for this checkout. Until resolved, the manual ritual for new shell scripts: `git add <script>.sh && git update-index --chmod=+x <script>.sh`. |
| WP-170 | Consolidate `_run_middleware` test helper into shared conftest | `tests/conftest.py`, `tests/test_wp105_mcp_http.py`, `tests/test_wp169_mcp_auth_allowlist.py` | Surfaced 2026-05-06 in WP-169 /simplify review. `_run_middleware` is defined independently in both `test_wp105_mcp_http.py` (signature: `middleware, scope, token_header, api_key_header, api_keys`) and `test_wp169_mcp_auth_allowlist.py` (signature: `middleware, path, token_header, api_keys`). The WP-169 variant's `path` arg is a convenience wrapper around the scope dict. Extract a single canonical `_run_middleware(middleware, path, token_header, api_key_header, api_keys)` into `tests/conftest.py` or a `tests/helpers/mcp_auth.py` module and update both callers. Low-urgency; pick up the next time either test file is touched. |
| WP-172 | Add `?include_archived=false` filter to `GET /knowledge/threats` | `cyber_knowledge/repo.py`, `cyber_knowledge/routes.py` (was `memory_service/knowledge_*.py` pre-WP-173) | Surfaced 2026-05-07 in WP-138b /simplify review (R4). `knowledge_repo.list_threats` returns all Threat nodes with no `archived` filter. After the WP-138b merge pass, `GET /knowledge/threats` returns archived nodes alongside live ones. Add `include_archived: bool = False` query param — default excludes archived nodes, which is the expected caller behaviour. Pick up when next touching `cyber_knowledge/routes.py`. |
| WP-171 | Align `hooks/stop.py` `API_BASE_URL` default with canonical fabric URL | `hooks/stop.py` | Surfaced 2026-05-06 in WP-156 follow-up (commit `71e1278`). `hooks/stop.py:44` defaults `API_BASE_URL` to `http://localhost:8000`, which only works inside WSL where the local Docker stack publishes that port. When Claude Code fires the Stop hook from the Windows side (now possible after the WP-156 follow-up made the registration cross-OS-portable), the script reaches `localhost:8000` from a Windows process — which never resolves to the WSL-side Memgraph — so the script's `httpx.ConnectError` branch fires and the close-session scaffold silently skips. The canonical fix mirrors the convention `memory_client/config.py` already uses (per memory `e5c17afa-...`, importance 4): default `API_BASE_URL` to `https://memfabric.carr-it.net:8443` so the no-config path always hits the externally-exposed canonical endpoint regardless of OS. Implementation: change line 44 to `API_BASE_URL = os.environ.get("API_BASE_URL", "https://memfabric.carr-it.net:8443")`. Trade-offs: (a) WSL-side runs will now traverse the public DNS path instead of `localhost:8000` — strictly fine but adds a network hop; (b) the `/health` probe at the canonical endpoint requires bearer auth as of 2026-04-20 (per memory `eaec3095-...`), so the script's bare `httpx.get` will receive 403 instead of 200 and silent-skip again — meaning the proper fix is a two-liner: load the bearer from `~/.memfabric/token` (canonical path per `a197148e-...`) and pass it as `Authorization: Bearer <token>` on the probe. Defer until the close-session scaffold is genuinely needed on Windows-side Claude Code; today the silent-skip is benign because the Mara baseline Stop hook still auto-captures via its own path. Pick up next time `hooks/stop.py` is touched. |
...
WP-138 calibrated the cross-report nearest-neighbour deduplication threshold to **0.28** (cosine distance, `paraphrase-multilingual-MiniLM-L12-v2` embeddings) and raised the `extract_cti_threats.py --dedup-threshold` default accordingly. New ingestion runs now dedupe correctly, but the **existing 364 Threat nodes** in the production graph were ingested under the old 0.15 default and still hold cross-report duplicate pairs that the new threshold would have merged.

This WP applies the calibrated threshold once, retrospectively, to bring the existing corpus to uniform state. Required before WP-113 wires `JEOPARDISES` (Threat → Precept) edges, otherwise the same precept will be jeopardised by N split-but-equivalent threats inflating apparent threat density.

#### Scope

- Run `find_duplicates`-style cross-report nearest-neighbour scan over all existing Threat nodes at threshold 0.28.
- For each pair above threshold: call the existing merge endpoint (the same path used by manual review). Canonical node selection: higher IDENTIFIES count wins; tie-break on creation timestamp (older wins). Loser node is archived (not deleted) with `merged_into` pointer per the standard merge semantics.
- Preserve `IDENTIFIES` edge provenance: all `IDENTIFIES` edges from the loser node are rewired to the canonical node, including `severity`/`confidence`/`trend`/`source_terminology` properties. Multiple ThreatReports identifying the same merged node is the expected post-condition.
- Preserve `MAPPED_TO_TECHNIQUE` edges: union of both nodes' technique mappings rewired to the canonical node, deduplicated by target Framework node.
- No re-embedding (embeddings are deterministic from text; merge does not change text).
- Maintenance log entry per merge (operation=`auto_merge_wp138b`, both node IDs, similarity score, surviving node ID).

#### Out of scope

- Changing the calibration threshold (use 0.28 from WP-138).
- Changing the merge algorithm (use existing manual merge code path).
- Retroactive changes to closed ThreatReport ingestion (the report is canonical; only the Threat-corpus shape changes).
- Threat→Precept JEOPARDISES wiring (that is WP-113).

#### Definition of Done

- Pre-run snapshot captured: count of Threat nodes, count of IDENTIFIES edges, count of MAPPED_TO_TECHNIQUE edges, list of (id, fact-excerpt) pairs that will merge.
- Merge pass executed against the production fabric.
- Post-run snapshot captured: same counts, plus the count of newly-archived Threat nodes with `merged_into` set.
- Maintenance log entries verified for every merge.
- Existing tests still pass (no regression in WP-108 or WP-138 test suites).
- BACKLOG.md updated: WP-138b ticked, WP-113's "Depends on" updated to `✅ WP-138b`. CHANGELOG.md write-back done.

#### Notes

- This is intentionally a one-off operational task, not a recurring job. Future ingestion runs use the new default and will dedupe at write time.
- If post-run merge count exceeds ~30 (more than ~8% of corpus), pause for manual review of a sample before completing — the WP-138 calibration was validated at "15 merges, all genuine paraphrases" so a much larger merge set might indicate a regression in the embedding model snapshot or a bad ThreatReport ingest.
...
#### Re-scoping (2026-05-06)

The original spec bundled three components and explicitly deferred `JEOPARDISES` to "WP-108 once Threat nodes exist". WP-108 is now closed (364 Threats live, per WP-138 ✅), so `JEOPARDISES` has no other natural home. The `ADDRESSES` (Control → Precept) edge requires a populated Control tree, which the fabric currently lacks (*"the gap-analysis engine exists but only has a test control"*). Component 3 (additional architectural frameworks) is genuinely independent work that should not block the strategic-path activation.

The corrected scope below makes WP-113 the WP that *activates the strategic path* end-to-end with the populated knowledge layer that exists today, and splits Component 3 into per-framework follow-on WPs (WP-113b–e). The plan agent may surface further amendments — treat the scope below as the current intent, not the final word.

#### Scope

**Component 1 — SABSA Business Attribute Profile + universal Precepts (full):**
...
- Derive ~30–50 universal Precept nodes from cross-framework convergence: obligations that ISO 27001, NIST CSF, COBIT 2019, and SP 800-53 all independently demand (access control, data protection, audit/logging, incident response, risk assessment, supply chain security, etc.) — use the existing INFORMS edge density (WP-107 cross-framework cluster analysis) as the convergence signal
- Create `FULFILS` edges: universal Precepts → Business Attributes

**Component 2 — Wire `REQUIRES` from Frameworks (partial; defer ADDRESSES):**
- Wire `REQUIRES` edges from existing **Framework leaf nodes** to Precepts. ADR-002 distinguishes Norms (prescriptive, with normative weight) from Frameworks (structural). The fabric currently holds ISO 27001 / NIST CSF / COBIT / SP 800-53 / ATT&CK as Frameworks (no real Norms loaded yet), so REQUIRES is wired Framework → Precept for now. When real Norms are introduced (e.g. NIS2, DORA, GDPR), they will get their own REQUIRES edges to the same Precepts, and the existing Framework→Precept edges remain as structural support.
- **Defer `ADDRESSES` (Control → Precept)** until a real Control tree is seeded. Either as a sibling task within this WP (small reference Control set) or as a follow-on WP — to be decided by the plan agent based on whether a useful reference Control set can be derived from existing Frameworks.

**Component 3 — `JEOPARDISES` (Threat → Precept) — moved into scope:**
- Wire `JEOPARDISES` edges from the 364 existing Threat nodes (post WP-138b dedup pass) to relevant Precept nodes. Two seeding strategies to evaluate in the plan: (a) embedding similarity between Threat.text and Precept.text, with manual review queue for borderline scores; (b) traversal-based — use existing Threat→MAPPED_TO_TECHNIQUE→Framework paths to find Frameworks that REQUIRES a Precept, then propose JEOPARDISES from the Threat to that same Precept (transitive). Likely best as a hybrid.
- Add `severity` and `rationale` properties to JEOPARDISES per ADR-002.
- Provenance tag any auto-inferred JEOPARDISES edges so they can be reviewed.
...
- `ADDRESSES` (Control → Precept) — deferred until a real Control tree exists (see Component 2)
- Additional architectural frameworks — split out as follow-on WPs (see WP-113b–e below)

---

### WP-113b — C2M2 (Cybersecurity Capability Maturity Model) ingestion

C2M2 has 10 domains with maturity-level descriptors. Useful for capability gap analysis alongside compliance gap analysis. To assess: whether ingesting as Framework nodes adds value vs treating as metadata on Control nodes. Wire INFORMS edges to existing Framework leaves where embedding similarity is high. Score: M / M / 1.0 (refine when the WP is planned).

---

### WP-113c — Zero Trust Architecture (NIST SP 800-207) ingestion

NIST SP 800-207 is principles rather than controls — more suitable as Precept-level content than as Framework nodes. Likely the cleanest pattern: extract the seven tenets and the related architectural assumptions as additional universal Precepts (added to WP-113's Component 1 set), with a small Framework spine for the document structure. Score: M / M / 1.0 (refine when the WP is planned).

---

### WP-113d — CIS Controls v8 ingestion

18 controls with implementation groups (IG1/IG2/IG3). Highly practical. Strong overlap with SP 800-53 and NIST CSF — likely to produce dense INFORMS edges. May serve as a better "glue" between ATT&CK and governance frameworks than SP 800-53 for some use cases. Wire INFORMS to ATT&CK and existing governance Frameworks. Score: M / M / 1.0 (refine when the WP is planned).

---

### WP-113e — TOGAF Security Architecture domain ingestion

Relevant for enterprise architecture alignment. Lower priority unless a TOGAF-aligned org context is added. Score: L / M / 0.33 (refine when the WP is planned).
...
_(WP-169 description block moved to `docs/CHANGELOG.md` on completion 2026-05-06.)_
