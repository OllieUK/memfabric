...
Added `mcp_server/_coercion.py` with a `BeforeValidator`-backed `StrList` type alias: `None` and real lists pass through unchanged, JSON-array strings are parsed via `json.loads`, bare strings wrap as single-element lists, and malformed JSON / non-array strings fall through to Pydantic's strict validator so genuine client bugs still surface. A WARNING is logged whenever coercion fires (silent on the no-op real-list path) so client misbehaviour stays visible. Applied `StrList` to all 17 `list[str] | None` parameters across `memory_add`, `memory_search`, `memory_update`, `memory_reinforce`, and `task_add` in `mcp_server/server.py`. The published JSON schema for these parameters remains `array | null` — MCP `tools/list` discovery output is identical for compliant clients. The factory `make_list_coercer(item_type)` is exposed for future non-`str` lists but unused in production.

**Tests:** 13 unit tests (U-1..U-13) cover `_coerce_str_list` semantics, the `StrList` Pydantic alias, the warning gate, and the `make_list_coercer` factory. 9 integration tests (I-1..I-9) exercise the full path through the MCP HTTP transport against the live stack — `memory_add` with JSON-string `strand_ids` (creates a memory, threads it to the strand, verified via direct Cypher), with a real list (regression guard), with a bare string, plus `memory_search` JSON-string `tags`, `memory_update` JSON-string `strand_ids`, `memory_reinforce` JSON-string `co_recalled_ids`, and `task_add` JSON-string `memory_ids`. Manual smoke test from curl: `memory_add` with `strand_ids='["strand-test"]'` (JSON-encoded) returned `memory_id` and a verified `IN_STRAND` edge to `strand-test`.

**Side action (out of scope):** File an upstream bug report with Anthropic about Cowork's MCP bridge double-encoding array params.

**Retrospective:** Plan was followed verbatim with two minor mid-flight adjustments. (1) The original `_coerce_str_list` shape only triggered the JSON-parse branch when the string both started with `[` and ended with `]`; the test for malformed JSON (`'[broken'`) showed that strings opening with `[` but missing the close were falling through into the bare-string branch and being coerced to `["[broken"]`. Fixed by switching the gate to "starts with `[`" only — `json.loads` is the real decision, and on failure we return the original so Pydantic reports the canonical type error. (2) The integration-test helper `_mcp_call` from WP-105 patterns failed against the current FastMCP build with `406 Not Acceptable: Client must accept both application/json and text/event-stream`; added the dual `Accept` header and an SSE-or-JSON parser. This is a pre-existing gap in the WP-105 integration-test scaffolding (its tests fail the same way today) and worth a separate small WP to fix. **Test infra note:** the live test stack started with no schema or strands; ran a one-off `tmp/test_setup_wp150.py` to create the constraints, vector index, and `strand-inbox`/`strand-test` Strand seeds. **What went well:** TDD cycle was clean — 13 unit tests written and failing before any implementation, then implementation, then watching them flip green; the only iteration was on U-6, which surfaced the `[broken` edge case the plan hadn't called out. **What to improve:** WP-105 integration tests need updating to send `Accept: application/json, text/event-stream` so the regression sweep has live coverage of the MCP HTTP transport.
...
**Retrospective:** This WP took four fix commits after the initial scaffolding (`2d20698` → `a701daf` → `3781ea0` → `63f9229`) because three genuinely independent failure modes masked each other. (1) `FastMCP.http_app(path="/mcp")` interacted badly with `app.mount("/mcp", ...)` to produce a 404 — fix was `path="/"` on the sub-app. (2) Passing a bare ASGI class in `middleware=[...]` silently worked in dev but `BearerTokenMiddleware` has a one-arg `__init__(app)` that FastMCP's factory call couldn't invoke correctly under stateless_http — fix was `Middleware(BearerTokenMiddleware)`. (3) The third-layer 500 (`StreamableHTTPSessionManager task group was not initialized`) only surfaced once auth was correct and the first real MCP `initialize` request came through, because FastMCP lazy-initialises the session manager on first POST. The debugging loop would have been impossible from Windows without the `claude-diag` SSH channel shipped as WP-148 — `ssh homeserver-diag mcp-probe` and `mcp-initialize` turned each layer's error into a 10-second roundtrip instead of a copy-paste chain. Stackdeploy submodule gitlink bumping also bit us once: rebuilding the container with fresh source did nothing until `publish-change.ps1` advanced the gitlink on `cit-home-stackdeploy`. **Follow-ups:** rotate the bearer token used during debugging; consider mounting at `/mcp/` explicitly to avoid the 307 redirect (minor, only matters for clients that don't follow redirects).

---

### WP-148: Homeserver SSH diagnostic channel for Claude — 2026-04-22

Narrow read-only SSH channel (`claude-diag` user on homeserver) that lets a Claude Code session on Windows run a fixed allow-list of diagnostic commands against the live stack without copy-paste or interactive shell. `scripts/homeserver/claude-diag` is a `ForceCommand` wrapper with allow-listed subcommands (`status`, `logs [N]`, `routes`, `mcp-probe`, `mcp-initialize`, `inspect-main`, `image-info`, `version`, `help`), a metacharacter guard, no `eval`, and env-var overrides for the stackdeploy layout. `docs/operations/homeserver-ssh-diag.md` documents the threat model, one-time install, sshd `Match User` block, Windows-side ssh_config entry, and the single-line revocation procedure. Blast radius if the key leaks: read container logs + status and probe `/mcp` from inside the container. No writes, no secrets echoed, no port forwarding.

**Retrospective:** Surfaced as an enabler during WP-105 debugging — three consecutive fix-deploy-inspect cycles were untestable from Windows without this. The instinct to use a generic SSH key was the wrong shape; the `ForceCommand` + `restrict` + allow-list + subcommand-dispatch shape makes the key leak-tolerant (revocable in one line, can only do read-only things) rather than relying on "don't lose the key". The `sudo -u` env-stripping gotcha (`SSH_ORIGINAL_COMMAND="..." sudo -u user ...` silently drops the var — must be `sudo -u user env SSH_ORIGINAL_COMMAND="..." ...`) cost a debugging cycle and is now documented in the ops doc. Separating server-side sshd directives (`AuthenticationMethods`, `PermitTTY`, etc.) from client-side `ssh_config` directives is called out in a prominent warning — putting sshd directives in Windows `~/.ssh/config` produces `Bad configuration option` parse errors. **Rule:** Never extend `/usr/local/bin/claude-diag` in place on homeserver; the repo is the source of truth and drift between them is how these things quietly grow teeth.
