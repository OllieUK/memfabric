...
class TestFrameworkRepo:
    def test_upsert_framework_runs_merge_query(self):
        ...
        req = MagicMock(id="fw-1", title="ISO 27001", version="2022",
                        parent_id=None, body=None, statement_type=None, modality=None)
        session = MagicMock()
        session.run.return_value.single.return_value = {
            "id": "fw-1", "title": "ISO 27001", "version": "2022",
            "level": "framework", "body": None,
            "statement_type": None, "modality": None,
    ...
    def test_upsert_framework_returns_dict(self):
        ...
        req = MagicMock(id="fw-1", title="ISO 27001", version=None,
                        parent_id=None, body=None, statement_type=None, modality=None)
        session = MagicMock()
        expected = {"id": "fw-1", "title": "ISO 27001", "version": None,
                    "level": "framework", "body": None,
                    "statement_type": None, "modality": None,
                    "created_at": "2024-01-01T00:00:00+00:00"}
...
class TestNormRepo:
    def test_upsert_norm_no_optional_edges(self):
        """upsert_norm with no maps_to_control_id/references_framework_id calls session.run once."""
        req = MagicMock(id="n-1", title="N1", body="Must encrypt",
                        level="article", version=None, valid_from=None, valid_until=None,
                        announced_at=None, text_hash=None, lang=None, domain=None,
                        maps_to_control_id=None, references_framework_id=None,
                        references_version_pinned=False)
        session = MagicMock()
        session.run.return_value.single.return_value = {
    ...
    def test_upsert_norm_creates_maps_to_edge(self):
        """upsert_norm with maps_to_control_id creates MAPS_TO edge (run called twice)."""
        req = MagicMock(id="n-1", title="N1", body="Must encrypt",
                        level="article", version=None, valid_from=None, valid_until=None,
                        announced_at=None, text_hash=None, lang=None, domain=None,
                        maps_to_control_id="c-1", references_framework_id=None,
                        references_version_pinned=False)
        session = MagicMock()
        session.run.return_value.single.return_value = {
            "id": "n-1", "title": "N1", "body": "Must encrypt",
        ...
        knowledge_repo.upsert_norm(session, req, [0.1], "2024-01-01T00:00:00+00:00")
        assert session.run.call_count == 2
        assert "MAPS_TO" in session.run.call_args_list[1][0][0]

    def test_upsert_norm_creates_references_edge(self):
        """upsert_norm with references_framework_id creates REFERENCES edge."""
        req = MagicMock(id="n-1", title="N1", body="Must encrypt",
                        level="article", version=None, valid_from=None, valid_until=None,
                        announced_at=None, text_hash=None, lang=None, domain=None,
                        maps_to_control_id=None, references_framework_id="fw-1",
                        references_version_pinned=True)
        session = MagicMock()
        session.run.return_value.single.return_value = {
            "id": "n-1", "title": "N1", "body": "Must encrypt",
            "level": "article", "version": None, "valid_from": None, "valid_until": None,
        ...
        knowledge_repo.upsert_norm(session, req, [0.1], "2024-01-01T00:00:00+00:00")
        assert session.run.call_count == 2
        assert "REFERENCES" in session.run.call_args_list[1][0][0]

    def test_upsert_norm_both_optional_edges(self):
        """upsert_norm with both maps_to_control_id and references_framework_id calls session.run three times."""
        req = MagicMock(id="n-1", title="N1", body="Must encrypt",
                        level="article", version=None, valid_from=None, valid_until=None,
                        announced_at=None, text_hash=None, lang=None, domain=None,
                        maps_to_control_id="c-1", references_framework_id="fw-1",
                        references_version_pinned=False)
        session = MagicMock()
        session.run.return_value.single.return_value = {
            "id": "n-1", "title": "N1", "body": "Must encrypt",
...
class TestChunkRepo:
    def test_upsert_chunk_creates_has_chunk_edge(self):
        ...
        req = MagicMock(id="ch-1", body="foo", sequence=0,
                        doc_id="doc-1", heading=None, section_ref=None,
                        status="unmatched", prev_chunk_id=None)
        session = MagicMock()
        session.run.return_value.single.return_value = {
            "id": "ch-1", "body": "foo", "sequence": 0,
            "doc_id": "doc-1", "heading": None, "section_ref": None,
    ...
    def test_upsert_chunk_creates_has_next_edge(self):
        ...
        req = MagicMock(id="ch-2", body="bar", sequence=1,
                        doc_id="doc-1", heading=None, section_ref=None,
                        status="unmatched", prev_chunk_id="ch-1")
        session = MagicMock()
        session.run.return_value.single.return_value = {
            "id": "ch-2", "body": "bar", "sequence": 1,
            "doc_id": "doc-1", "heading": None, "section_ref": None,
            "status": "unmatched", "created_at": "2024-01-01T00:00:00+00:00"
        }
        knowledge_repo.upsert_chunk(session, req, [0.1], "2024-01-01T00:00:00+00:00")
...
class TestKnowledgeRoutes:
    def test_post_framework_returns_200(self, app_client):
        ...
        assert resp.json()["title"] == "ISO 27001"
    ...
    def test_get_framework_returns_200(self, app_client):
        client, session = app_client
        session.run.return_value.single.return_value = {
            "id": "fw-1", "title": "ISO 27001", "version": None,
            "level": "framework", "body": None,
            "statement_type": None, "modality": None,
            "created_at": "2024-01-01T00:00:00+00:00"
        }
    ...
    def test_post_norm_embeds_body_field(self, app_client):
        """POST /knowledge/norms calls get_embedding with req.body."""
        client, session = app_client
        session.run.return_value.single.return_value = {
            "id": "n-1", "title": "N1", "body": "Must encrypt",
            "level": "article", "version": None, "valid_from": None, "valid_until": None,
    ...
    def test_post_document_invalid_policy_level_returns_400(self, app_client):
        """POST /knowledge/documents with invalid policy_level returns 400."""
        client, session = app_client
        resp = client.post("/knowledge/documents", json={
            "id": "doc-1", "title": "Policy", "policy_level": "invalid-level"
        })
        assert resp.status_code == 400

    def test_post_chunk_calls_get_embedding(self, app_client):
        """POST /knowledge/chunks calls get_embedding with req.body."""
        client, session = app_client
        session.run.return_value.single.return_value = {
            "id": "ch-1", "body": "Chapter 1", "sequence": 0,
            "doc_id": "doc-1", "heading": None, "section_ref": None,
            "status": "unmatched", "created_at": "2024-01-01T00:00:00+00:00"
        }
        with patch("memory_service.knowledge_routes.get_embedding",
    ...
    def test_post_chunk_invalid_status_returns_400(self, app_client):
        """POST /knowledge/chunks with invalid status returns 400."""
        client, session = app_client
        resp = client.post("/knowledge/chunks", json={
            "id": "ch-1", "body": "Chapter 1", "sequence": 0,
            "doc_id": "doc-1", "status": "invalid-status"
        })
        assert resp.status_code == 400
